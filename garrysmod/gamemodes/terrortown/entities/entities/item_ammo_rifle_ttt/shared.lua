   ...
   if SERVER and self.taken != true then
      if (ent:IsValid() and ent:IsPlayer() and self:CheckForWeapon(ent) and self:PlayerCanPickup(ent)) then
         ...
         local ammo = ent:GetAmmoCount(self.AmmoType)
			 ...
			 if (ent:GetWeapon("weapon_ttt_ak47"):IsValid() or ent:GetWeapon("weapon_ttt_an94"):IsValid() or ent:GetWeapon("weapon_ttt_f2000"):IsValid() or ent:GetWeapon("weapon_ttt_g36c"):IsValid() or ent:GetWeapon("weapon_ttt_aug"):IsValid() or ent:GetWeapon("weapon_ttt_sg552"):IsValid()) and (60 >= (ammo + math.ceil(30 * 0.25))) then
